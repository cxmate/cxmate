...
func (m *Mate) serve() {
	...
	srv := &http.Server{
		...
		WriteTimeout: m.Config.General.WriteTimeout,
		IdleTimeout:  m.Config.General.IdleTimeout,
		Handler:      mux,
	}
	m.Logger.Infoln("cxMate now listening on", m.Config.General.Location, "connected to service on", m.Config.Service.Location)
	m.Logger.Fatalln(srv.ListenAndServe())
}
